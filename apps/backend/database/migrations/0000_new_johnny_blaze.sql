...
CREATE TABLE `subscriptions` (
	...
	FOREIGN KEY (`creator_address`) REFERENCES `accounts`(`address`) ON UPDATE no action ON DELETE no action,
...
--> statement-breakpoint
CREATE INDEX `idx_subscriptions_creator` ON `subscriptions` (`creator_address`);--> statement-breakpoint
